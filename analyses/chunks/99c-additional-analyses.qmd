...
constant_reporters <- jsonlite::fromJSON(
  here::here("data/processed/constant_reporters.json")
)
...
hq_json <- jsonlite::fromJSON(
  here::here("data/processed/top_quality_documents_yearly.json")
)
...
if (file.exists(file.path(datasets_dir, "firm_data.csv"))) {
    ...
    make_violin_plot <- function(df, x_label, bracket_y = c(0.65, 0.7)) {
        ...
        group_levels <- c("Bottom 10%", "Middle 80%", "Top 10%")
        
        # Build labels in the same order as group_levels
        n_by_group <- df |>
            dplyr::count(group) |>
            tibble::deframe()  # named vector: group -> n
    
        group_labels <- sapply(group_levels, function(g) {
            n <- n_by_group[[g]]
            if (is.null(n)) n <- 0
            sprintf("%s<br>(*n*&nbsp;=&nbsp;%s)", g, format(n, big.mark = ","))
        })

        ggplot2::ggplot(df, ggplot2::aes(x = factor(group, levels = group_levels), y = transp)) +
            ...
            ggplot2::theme(
                ...
                axis.text.x = ggtext::element_markdown(lineheight = 1.1),
                legend.position = "none",
