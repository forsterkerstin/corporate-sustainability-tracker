...
# Set to TRUE to drop cells whose model_output repeats a prompt-example literal
FILTER_BLEED <- FALSE
...
# The numeric literals shown to the model in the config.yaml prompt example
PROMPT_ARTIFACTS <- c(281002.0, 1118.0, 15.4)

drop_bleed <- function(value_final, model_output) {
    if (!FILTER_BLEED) {
        return(value_final)
    }
    raw <- as.numeric(stringr::str_match(model_output, "'value':\\s*([0-9.eE+-]+)")[, 2])
    if_else(raw %in% PROMPT_ARTIFACTS, NA_character_, value_final)
}
...
df <- read_csv(file.path(results_dir, "esg_indicators_postprocessed.csv")) |>
    ...
    mutate(
        ...
        value_final = drop_bleed(value_final, model_output),
...
renamer_soc <- c(
    ...
    "Percentage of employees at top management level: Female" = "Female employees in top management [%]",
